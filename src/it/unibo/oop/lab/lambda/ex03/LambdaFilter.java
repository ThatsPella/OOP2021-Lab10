...
public final class LambdaFilter extends JFrame {
    ...
    private enum Command {
        ...
        LOWERCASE("Only lowercases", s -> s.toLowerCase()),
        NCHARACTERS("Number of characters", s -> Integer.toString(s.length())),
        NLINES("Number of lines", s -> Long.toString(s.chars().filter(c -> c == '\n').count())),
        WORDLIST("List of all ordered words", s -> Arrays.stream(s.split(ANY_NON_WORD)).sorted().collect(Collectors.joining("\n"))),
        WORDCOUNT("Count words", s -> Arrays.stream(s.split(ANY_NON_WORD)).collect(Collectors.groupingBy(Function.identity(), Collectors.counting())).entrySet().stream().map(e -> e.getKey() + " -> " + e.getValue()).collect(Collectors.joining("\n")));
